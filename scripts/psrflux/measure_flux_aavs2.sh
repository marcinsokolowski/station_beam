...
echo "python ~/github/station_beam/python/eda_sensitivity.py --bandwidth $bw --frequency $freq --unixtime $uxt --ra=$rajd --dec=$decjd --inttime=$inttime --outsens_file=uxtime${uxt}_aavs2_sensitivity --antenna_locations=${HOME}/aavs-calibration/config/aavs2/antenna_locations.txt  --outfile_mode=a --trcv_type=trcv_from_skymodel_with_err --nos11 --header=HEADER --use_beam_fits --station_name=SKALA4 --size=512 --trcv_type=trcv_aavs2_vs_za_deg -p None -m analytic > ${src}_sensitivity.out 2>&1"
python ~/github/station_beam/python/eda_sensitivity.py \
          ...
          --antenna_locations=${HOME}/aavs-calibration/config/aavs2/antenna_locations.txt \
          --outfile_mode=a \
